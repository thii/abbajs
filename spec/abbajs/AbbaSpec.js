var Abba = require('../../index.js').Abba;
...
describe('NormalDistribution', function() {
  ...
  it('computes density', function() {
    ...
    expectedDensities.forEach(function(values) {
      expect(normal.density(values[0])).toBeCloseTo(values[1], MAX_ERROR);
    });
  });
  ...
  it('computes CDFs and survival functions', function() {
    ...
    expectedCumulativeProbabilities.forEach(function(values) {
      expect(normal.cdf(values[0])).toBeCloseTo(values[1], MAX_ERROR);
      expect(normal.survival(values[0])).toBeCloseTo(1 - values[1], MAX_ERROR);
    });
  });
  ...
  it('computes inverse CDFs and survival functions', function() {
    ...
    expectedValues.forEach(function(values) {
      expect(normal.inverseCdf(values[0])).toBeCloseTo(values[1], MAX_ERROR);
      expect(normal.inverseSurvival(values[0])).toBeCloseTo(1 - (values[1] - 1), MAX_ERROR);
    });
  });
});
...
describe('BinomialDistribution', function() {
  ...
  it('computes mass', function() {
    ...
    expectedMass.forEach(function(values) {
      expect(binomial.mass(values[0])).toBeCloseTo(values[1], MAX_ERROR);
    });
  });
  ...
  it('computes CDFs and survival functions', function() {
    ...
    expectedCumulativeProbabilities.forEach(function(values) {
      expect(binomial.cdf(values[0])).toBeCloseTo(values[1], MAX_ERROR);
      expect(binomial.survival(values[0])).toBeCloseTo(1 - values[1], MAX_ERROR);
    });
  });
  ...
  it('computes inverse CDFs and survival functions', function() {
    ...
    expectedValues.forEach(function(values) {
      expect(binomial.inverseCdf(values[0])).toBeCloseTo(values[1], 0);
      expect(binomial.inverseSurvival(values[0])).toBeCloseTo(300 - (values[1] - 300), 0);
    });
  });
});
